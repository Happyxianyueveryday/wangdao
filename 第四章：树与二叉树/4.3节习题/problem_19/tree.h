...
class Tree
{
    ...
    vector<int> postorder1() const;            // 非递归的后序遍历
    vector<int> postorder2() const;            // 非递归的后序遍历
	...
	TreeNode *getRoot();                      // 测试用方法: 获取当前树的根结点
...
*/
vector<int> Tree::postorder1() const
{
    // 0. 处理特殊情况
    vector<int> res;
    if(!treeroot)
...
/*
 *  postorder2: 二叉树的非递归后序遍历的另外一种写法
 *  note: 因为后序遍历的顺序是: 左子树->右子树->根结点，而前序遍历的顺序是: 根结点->左子树->右子树，因此只要对前序遍历的程序稍作调整，调整到顺序为: 根结点->右子树->左子树，然后进行调整顺序后的前序遍历，最后将得到的结果反转即可得到后序遍历序列 
*/
vector<int> Tree::postorder2() const
{
    // 0. 处理特殊情况
    vector<int> res;
    if(!treeroot)
	...
	while(sta.size())
	{
		...
		if(now->left)    // 因为要调整前序遍历的顺序为: 根结点->右子树->左子树，因此左子树先入栈 
		sta.push(now->left);
		if(now->right)
		sta.push(now->right);
	}
